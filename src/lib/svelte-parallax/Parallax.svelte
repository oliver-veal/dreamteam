<script>
  ...
  const scrollTop = derived([y, top], ([$y, $top], set) => {
    const dy = $y - $top;
    const min = 0 - outerHeight + outerHeight * enter;
    const max = outerHeight * sections - outerHeight * exit;
    // sorry
  ...
  $: $layers.forEach((layer) => {
    layer.setHeight(outerHeight);
  });
  $: $layers.forEach((layer) => {
    layer.setPosition($scrollTop, outerHeight, disabled);
  });
  ...
  export function scrollTo(
    ...
    const scrollTarget = $top + outerHeight * (section - 1);
